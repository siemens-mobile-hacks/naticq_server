...
//  inline bool addIgnore(string uin, string nick="") { if (!removeContact(uin)) return false; return addVisInvisIgnor(IgnoreList, 0x000E, uin, nick); }
  ...
  virtual void onClientIDNotify(string from, uint8_t clientid) {}
 ...
 private:
  ...
  bool parseOnlineNotify(string & uin, uint32_t & stat, vector<uint8_t> & data, vector<uint8_t> &xs, uint8_t & clientid);
  ...
  bool parseXtrazResponse(string & from, vector<uint8_t> & x_status, string & x_title, string & x_descr, vector<uint8_t> & data);
  ...
  bool haveXtrazCapability(vector<uint8_t> & data, vector<uint8_t> & xs);
//  void sendXtrazRequest(string uin);
  void sendXStatusNotifyAutoResponse(string touin, uint8_t * msg_cookie);
  bool haveIGCapability(vector<uint8_t> & data);
  uint8_t getClientID(vector<uint8_t> & data);
