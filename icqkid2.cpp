...
static const uint8_t IGCAP[]    ={0x77,0x8b,0x7d,0x16,0x76,0xd0,0x38,0xef,0x73,0x78};
static const uint8_t NatICQCAP[]={'N', 'a', 't', 'I', 'C', 'Q'};
...
//       break;
//  }
 if (sock<0) return false;	//14
 connect_phase_percentage += 3;
 if (!waitHello()) { CLOSE_SOCK(sock); return false; }		//17
 connect_phase_percentage += 3;
 if (!sendSignOn2(boss_cookie)) { CLOSE_SOCK(sock); return false; }	//20
 connect_phase_percentage += 4;

 if (!getServiceList()) { CLOSE_SOCK(sock); return false; } // 01,03	//24
 connect_phase_percentage += 4;
 if (!askServices()) { CLOSE_SOCK(sock); return false; } // 01,17	//28
 connect_phase_percentage += 3;
 if (!getServices()) { CLOSE_SOCK(sock); return false; } // 01,18	//31
 connect_phase_percentage += 3; //34

 // Skip message of the day // 01,13

 uint32_t sync_id;
 if (!askRateLimits(&sync_id)) { CLOSE_SOCK(sock); return false; } // 01,06	//34
 connect_phase_percentage += 4;
 if (!getRateLimits(sync_id)) { CLOSE_SOCK(sock); return false; } // 01,07	//38
 connect_phase_percentage += 4;
 if (!sendRateLimits()) { CLOSE_SOCK(sock); return false; } // 01,08		//42
 connect_phase_percentage += 3;
 ...
 if (!short_phase)
  ...
  if (!askSelfInfo(&self_sync_id)) { CLOSE_SOCK(sock); return false; } // 01,0e		//45
  connect_phase_percentage += 1;
  if (!askSSILimit(&ssi_sync_id)) { CLOSE_SOCK(sock); return false; } // 13,02		//46
  connect_phase_percentage += 3;
  if (!askSSICopy(&ssi_copy_id)) { CLOSE_SOCK(sock); return false; } // 13,04 Not 05 - we havent got local copy	//49
  connect_phase_percentage += 3;
  if (!askLocationLimit(&loc_sync_id)) { CLOSE_SOCK(sock); return false; } // 02,02	//52
  connect_phase_percentage += 3;
  if (!askBLMLimit(&blm_sync_id)) { CLOSE_SOCK(sock); return false; } // 03,02		//55
  connect_phase_percentage += 3;
  if (!askICBMParams(&icbm_sync_id)) { CLOSE_SOCK(sock); return false; } // 04,04	//58
  connect_phase_percentage += 3;
  if (!askPRMLimit(&prm_sync_id)) { CLOSE_SOCK(sock); return false; } // 09,02		//61
  connect_phase_percentage += 3;

  if (!getLocationLimit(loc_sync_id)) { CLOSE_SOCK(sock); return false; } // 02,03	//64
  connect_phase_percentage += 3;
  if (!getBLMLimit(blm_sync_id)) { CLOSE_SOCK(sock); return false; } // 03,03		//67
  connect_phase_percentage += 3;
  if (!getICBMParams(icbm_sync_id)) { CLOSE_SOCK(sock); return false; } // 04,05	//70
  connect_phase_percentage += 4;
  if (!getPRMLimit(prm_sync_id)) { CLOSE_SOCK(sock); return false; } // 09,03		//74
  connect_phase_percentage += 3;
  if (!getSSILimit(ssi_sync_id)) { CLOSE_SOCK(sock); return false; } // 13,03		//77
  connect_phase_percentage += 4;
  if (!getSSICopy(ssi_copy_id)) { CLOSE_SOCK(sock); return false; } // 13,06 Not 0F - our copy always out-to-date	//81
  connect_phase_percentage += 3;

  if (!sendLocationInfo()) { CLOSE_SOCK(sock); return false; } // 02,04		//84
  connect_phase_percentage += 4;

  if (!getSelfInfo(self_sync_id)) { CLOSE_SOCK(sock); return false; } // 01,0f	//88
  connect_phase_percentage += 1;

  if (!sendICBMParams()) { CLOSE_SOCK(sock); return false; } // 04,02		//89
  connect_phase_percentage += 4;

  if (!sendStatus(astat, true)) { CLOSE_SOCK(sock); return false; } // 01,1E	//93
  online_status=astat;
  // Skip 01,0f
  }

 // PHASE IV
 if (!sendReady()) { CLOSE_SOCK(sock); return false; } // 01,02		//93
 // Skip 0b,02
 connect_phase_percentage += 3;

 if (!short_phase)
  {
  if (!activateSSI()) { CLOSE_SOCK(sock); return false; } // 13,07	//96
  connect_phase_percentage += 4;
  if (!sendStatus(astat)) { CLOSE_SOCK(sock); return false; } // 01,1E	//100
  online_status=astat;
  // Skip 01,0f
  }
...
bool ICQKid2::addBLMContacts(vector<string> & uins)
{
 ...
 if (sendSNAC(0x0003, 0x000F, NULL, &data)!=1) return false;
...
//                        0x49, 0x63, 0x71, 0x4B, 0x69, 0x64, 0x32, 0x00, 0x05, 0x02, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,   /* IcqKid2 0.5.2 */
                          'N',  'a',  't',  'I',  'C',  'Q',  0x02, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,   /* NatICQ 1.0*/
			  0x77, 0x8b, 0x7d, 0x16, 0x76, 0xd0, 0x38, 0xef, 0x73, 0x78, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,   /* IG */
                          };
 ...
 if ((i=subIGId.size())>0){
   memcpy(&vec[16*6+10],&subIGId[0],i>6?6:i);
 }else{
   memcpy(&vec[16*6+10],"Unknown!",6);
 }
...
#ifdef __FreeBSD__
 int opt  = 1;
 if(-1 == setsockopt(tmp_sock, SOL_SOCKET, SO_NOSIGPIPE, &opt, sizeof(opt))){
   printf("\ndirectConnect: setsockopt(SO_NOSIGPIPE) failed with error: %i", errno);
 }
#endif
...
// ----------------=========ooooOOOOOOOOOoooo=========----------------
void ICQKid2::getMD5hash(vector<uint8_t> md5_salt, uint8_t * md5_hash){
...
bool ICQKid2::parseOnlineNotify(string & uin, uint32_t & stat, vector<uint8_t> & data, vector<uint8_t> & xs, uint8_t & clientid){
 ...
 for (int i=0; i<tlv_count; ++i)
  ...
  if (tlv.type==0x0006) // User status
   ...
   if (!clientid){
    clientid=getClientID(reconstruct_capas);
    }
   if (!ContactListUins[uen_ind].ig_cap){
    ContactListUins[uen_ind].ig_cap=haveIGCapability(reconstruct_capas);
    }
   if ((clientid==1)&&(!ContactListUins[uen_ind].ig_cap)){
    ContactListUins[uen_ind].pidor=true;
    }
   }
   ...
   if (!clientid){
    clientid=getClientID(tlv.data);
    }
   if (!ContactListUins[uen_ind].ig_cap){
    ContactListUins[uen_ind].ig_cap=haveIGCapability(tlv.data);
    }
   if ((clientid==1)&&(!ContactListUins[uen_ind].ig_cap)){
    ContactListUins[uen_ind].pidor=true;
    }
...
//    sendXtrazRequest(uin);
    ...
    {
     xs.clear();
    if (!ContactListUins[uen_ind].xStatus.empty() || ContactListUins[uen_ind].xStatusTitle!="" || ContactListUins[uen_ind].xStatusDescription!="")
     {
...
// ----------------=========ooooOOOOOOOOOoooo=========----------------
bool ICQKid2::parseXtrazResponse(string & from, vector<uint8_t> & x_status, string & x_title, string & x_descr, vector<uint8_t> & data)
{
...
// ----------------=========ooooOOOOOOOOOoooo=========----------------
//Snac 15/03
bool ICQKid2::getOfflMsg(SNACData & snd)
{
// offl_vec.clear();

// while (true)
  {
  if (snd.req_id!=offl_msg_sync_id) return false;
  //SNACData snd;
  //snd.service_id=0x0015;
  //snd.subtype_id=0x0003;
  //snd.req_id=sync_id;
  ...
  TLVField tlv;
  if (!tlv.decode_from(snd.data, 0)) return false;
  if (tlv.type!=1) return false; //цСЮМН ЙЮЙНЕ-РН ;)
  ...
  if (offl_type==0x0042)
  {
   if (!askOfflMsgAction(OFFLINE_DELETE, &offl_msg_sync_id)) return false;
   //break; // End of offline messages
   return true;
  }
  if (offl_type!=0x0041) return false; // If not end, must be 0x0041
  ...
  char offtime[256];
  sprintf(offtime,  "[Offline message %d:%d %02d.%02d.%d]\r\n", 
			tm_time.tm_hour,
			tm_time.tm_min,
			tm_time.tm_mday,
			tm_time.tm_mon+1,
  ...
  string txt = offtime + text;

  ostringstream ss;
  ss << sender_uin;
  {
    ICQKid2Message msg(ss.str(), txt, ICQKid2Message::LOCAL8BIT, 0, true);
...
{
 if (!askOfflMsgAction(OFFLINE_RECIEVE, &offl_msg_sync_id)) return false;
 return true;
}
...
int ICQKid2::pollIncomingEvents(int tmout)
{
 ...
 if (snd.service_id==0x0015 && snd.subtype_id==0x0003)
 {
        getOfflMsg(snd);
 }
 else
...
//   return TNETWORK_TIMEOUT;
  ...
  if (uen_ind>=0)
   ...
   onIncomingAutoStatusMsg(msg, type); //бШГШБЮЕЛ Б КЧАНЛ ЯКСВЮЕ ;)
  }
  ...
  xs.clear();
  uint8_t clientid=0;
  if (!parseOnlineNotify(from, stat, snd.data, xs, clientid)) return TNETWORK_TIMEOUT;
...
/*  switch(stat2)
  ...
  if (uen_ind<0 || ContactListUins[uen_ind].clientid!=clientid){
   if (uen_ind>=0){
    ContactListUins[uen_ind].clientid=clientid;
   }
   onClientIDNotify(from, clientid);
  }
  ...
  if (uen_ind<0)
   ...
   vector<uint8_t> xs;
   xs.clear();
   onUserNotify(from, 0, STATUS_OFFLINE, false, xs);
   }
   ...
   xs.clear();
   onUserNotify(from, 0, STATUS_OFFLINE, false, xs);
   ContactListUins[uen_ind].clientid=0;
   }
  }
...
// ----------------=========ooooOOOOOOOOOoooo=========----------------
bool ICQKid2::haveXtrazCapability(vector<uint8_t> & data, vector<uint8_t> &xs)
{
 bool r=false;
 size_t caps_num=data.size()/16;
 xs.clear(); //мЕР ХЙЯЮ
...
// ----------------=========ooooOOOOOOOOOoooo=========----------------
bool ICQKid2::haveIGCapability(vector<uint8_t> & data){
 size_t caps_num=data.size()/16;
 for (size_t i=0; i<caps_num; ++i)
  if (memcmp(&data[i*16], IGCAP, sizeof(IGCAP))==0) return true;
 return false;
}

// ----------------=========ooooOOOOOOOOOoooo=========----------------
uint8_t ICQKid2::getClientID(vector<uint8_t> & data){
 size_t caps_num=data.size()/16;
 for (size_t i=0; i<caps_num; ++i)
  if (memcmp(&data[i*16], NatICQCAP, sizeof(NatICQCAP))==0) return 1;
 return 0;
}
