...
bool FlapPacket::recv_from(int sock)
{
 ...
 {
  tv.tv_sec=0;
  tv.tv_usec=300000; //ќжидание 300мс
  FD_ZERO(&rfds);
...
{
 vector<uint8_t> tmp_buf(6+payload.size());
 tmp_buf[0]='*';
 ...
 if (payload.size())
 {
        memcpy(&tmp_buf[6],&payload[0], payload.size());
 }
 t_network_error=TNETWORK_ERR;
 if ((unsigned int)send(sock, (char*)&tmp_buf[0], 6+payload.size(), 0)!=(6+payload.size())) return false;
// if(payload.size())
...
#define UNPACK_LE(arg) \
 ...
 if (msg_type==0x01) // Plain message optional data
  ...
  if(pos<data.size()) text_guid.assign((const char *)&data[pos], text_guid_len);
//  pos+=text_guid_len;
  }
...
bool TLV2711::parseXtrazResponse(void)
{
  ...
  x_status.insert(x_status.end(),atoi(str_ind.c_str()));
